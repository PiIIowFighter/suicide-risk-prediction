...
class BERT_TextCNN_BiLSTM(nn.Module):
    def __init__(self):
        ...
        self.cnn_linear = nn.Linear(HIDDEN_DIM * 3, HIDDEN_DIM)
    ...
    def forward(self, input_tensor, mask, hidden):
        ...
        lstm_out, (hidden_last, cn_last) = self.lstm(sequence_output,
                                                     hidden)  # lstm_out: (batch, seq_len, HIDDEN_DIM*2)

        # Instead of attention, use the final hidden states from BiLSTM
        # Concatenate the final forward and backward hidden states
        final_hidden = torch.cat((hidden_last[-2], hidden_last[-1]), dim=1)  # (batch, HIDDEN_DIM*2)

        # Project LSTM output to HIDDEN_DIM
        lstm_proj = self.lstm_linear(final_hidden)  # (batch, HIDDEN_DIM)
        ...
        # Feature fusion
        combined_output = weight_cnn * cnn_proj + weight_lstm * lstm_proj  # (batch, HIDDEN_DIM)

        # Apply dropout
        all_out = self.dropout(combined_output)  # (batch, HIDDEN_DIM)
