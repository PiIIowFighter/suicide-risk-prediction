...
class BERT_TextCNN_BiLSTM_IG(nn.Module):
    ...
    def forward(self, input, mask, hidden):
        ...
        cnn_out1 = self.conv_and_relu(self.conv1, bert_out.unsqueeze(1))  # (batch, hidden_dim, new_seq_len, 1)
        cnn_out2 = self.conv_and_relu(self.conv2, bert_out.unsqueeze(1))  # (batch, hidden_dim, new_seq_len, 1)
        cnn_out3 = self.conv_and_relu(self.conv3, bert_out.unsqueeze(1))  # (batch, hidden_dim, new_seq_len, 1)


        cnn_out1 = cnn_out1.squeeze(-1)  # (batch, hidden_dim, new_seq_len)
        cnn_out2 = cnn_out2.squeeze(-1)  # (batch, hidden_dim, new_seq_len)
        cnn_out3 = cnn_out3.squeeze(-1)  # (batch, hidden_dim, new_seq_len)


        cnn_out = torch.cat([cnn_out1, cnn_out2, cnn_out3], dim=1)  # (batch, hidden_dim*3, new_seq_len)


        cnn_out = cnn_out.permute(0, 2, 1)

        lstm_out, (hidden_last, cn_last) = self.lstm(cnn_out, hidden)  # lstm_out: (batch, new_seq_len, hidden_dim*2)


        ig_weights = torch.sigmoid(self.info_gain(lstm_out))  # (batch, new_seq_len, 1)
        weighted_lstm_out = lstm_out * ig_weights  # (batch, new_seq_len, hidden_dim*2)


        ig_out = torch.sum(weighted_lstm_out, dim=1)  # (batch, hidden_dim*2)

        out = self.dropout(ig_out)
